...
export const checkServiceHealth = async (serviceUrl: string): Promise<void> => {
  try {
    ...
    const details = error.response?.status ? `Service responded with status code: ${error.response.status}` : `Service with the URL ${serviceUrl} is not reachable`;
    throw new HttpError({ message, details, statusCode: error.response?.status || 500 });
  }
};
